...
public class DDSCompressor
{
    ...
    public static unsafe byte[] DecompressBlock(Byte[] data, int width, int height, DDS.DXGI_FORMAT format)
    {
        ...
        //new DirectXTexNet.TexHelperImpl();

        long inputRowPitch = 1;
        long inputSlicePitch = 1;
        //DirectXTexNet.TexHelper.Instance.ComputePitch((DirectXTexNet.DXGI_FORMAT)format, width, height, out inputRowPitch, out inputSlicePitch, 0);

        ulong rowPitch;
        ulong slicePitch;

        // Aufruf der ComputePitch-Methode
        int r;
        unsafe
        {
            //r = Hexa.NET.DirectXTex.DirectXTex.ComputePitch((int)format, (ulong)width, (ulong)height, (ulong*)&inputRowPitch, (ulong*)&inputSlicePitch, 0);
            FormatHelper.ComputePitch((DXGI_FORMAT)format, width, height, ref inputRowPitch, ref inputSlicePitch, CPFlags.BadDxtnTails);
        }
        ...
        {
            Hexa.NET.DirectXTex.Image hexaInput = new Hexa.NET.DirectXTex.Image((ulong)width, (ulong)height, (int)format, (ulong)inputRowPitch,0, (byte*)pData);

            Hexa.NET.DirectXTex.ScratchImage scratchImage = new Hexa.NET.DirectXTex.ScratchImage();
            scratchImage.InitializeFromImage(hexaInput, false, CPFlags.None);

            DirectXTex.Decompress(hexaInput, (int)FormatDecompressed, scratchImage);

            Marshal.Copy((nint)scratchImage.GetPixels(), result, 0, result.Length);

            scratchImage.Release();
        }*/

        /*Hexa.NET.DirectXTex.ScratchImage image = DirectXTex.CreateScratchImage();
        image.Initialize(new Hexa.NET.DirectXTex.TexMetadata((ulong)width, (ulong)height, depth: 1, arraySize: 1, mipLevels: 1, miscFlags: 0, miscFlags2: 0, (int)format, TexDimension.Texture2D), CPFlags.None);

        fixed (byte* pData = data)
        {
            var dstImg = image.GetImage(0, 0, 0);
            Buffer.MemoryCopy(pData, image.GetImage(0, 0, 0).Pixels, (long)dstImg.RowPitch * (long)dstImg.Height, data.Length);

            Hexa.NET.DirectXTex.ScratchImage outScImage = DirectXTex.CreateScratchImage();
            DirectXTex.Decompress2(image.GetImages(), image.GetImageCount(), image.GetMetadata(), (int)FormatDecompressed, outScImage);
        }*/

        Hexa.NET.DirectXTex.ScratchImage image = DirectXTex.CreateScratchImage();
        image.Initialize(new Hexa.NET.DirectXTex.TexMetadata((ulong)width, (ulong)height, depth: 1, arraySize: 1, mipLevels: 1, miscFlags: 0, miscFlags2: 0, (int)format, Hexa.NET.DirectXTex.TexDimension.Texture2D), Hexa.NET.DirectXTex.CPFlags.None);
        ...
        Hexa.NET.DirectXTex.ScratchImage decompressedImage = DirectXTex.CreateScratchImage();
        int hResult = DirectXTex.Decompress2(image.GetImages(), image.GetImageCount(), image.GetMetadata(), (int)FormatDecompressed, decompressedImage);
        image.Release();
        ...
        /*var decompressed = scratchImage.Decompress(0, FormatDecompressed);
        ...
        /*using (var decomp = hexaInput.Decompress(0, FormatDecompressed))
            {
                byte[] result = new byte[4 * width * height];
    ...
    public static byte[] Convert(byte[] data, int width, int height, DXGI_FORMAT inputFormat, DXGI_FORMAT outputFormat)
    {
        ...
        if (data.Length == inputSlicePitch)
        {
            var image = Hexa.NET.DirectXTex.DirectXTex.CreateScratchImage();
            int iResult = Hexa.NET.DirectXTex.DirectXTex.Initialize(image,
                new Hexa.NET.DirectXTex.TexMetadata((ulong)width, (ulong)height, 1, 1, 1, 0,
                    0, (int)inputFormat, Hexa.NET.DirectXTex.TexDimension.Texture2D), Hexa.NET.DirectXTex.CPFlags.None);

            var convFlags = Hexa.NET.DirectXTex.TexFilterFlags.Default;
            ...
            {
                convFlags |= Hexa.NET.DirectXTex.TexFilterFlags.Srgb;
            }
            ...
            unsafe
            {
                ...
                Hexa.NET.DirectXTex.ScratchImage convertedImage = Hexa.NET.DirectXTex.DirectXTex.CreateScratchImage();
                int cResult = Hexa.NET.DirectXTex.DirectXTex.Convert2(image.GetImages(), image.GetImageCount(),
                    image.GetMetadata(), (int)outputFormat, convFlags, 0.5f, convertedImage);
                image.Release();
